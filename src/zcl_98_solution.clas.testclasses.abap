...
CLASS ltcl_find_flights  DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    CLASS-DATA some_flight_data TYPE z98_cargo_flight.
...
*    IF sy-subrc <> 0.
*
*    cl_abap_unit_assert=>fail( 'Keine Daten in der Tabelle Z98_CARGO_FLIGHT'  ).
*
*    ENDIF.
*
*   TRY.
...
*       cl_abap_unit_assert=>fail( 'lcl_carrier kann nicht instanziert werden' ).
*
*   ENDTRY.
  ...
  METHOD class_setup.
   ...
   FIELDS carrier_id, connection_id, flight_date,
    ...
    IF sy-subrc <> 0.
      cl_abap_unit_assert=>fail( `No data in table Z98_CARGO_FLIGHT` ).
    ENDIF.
